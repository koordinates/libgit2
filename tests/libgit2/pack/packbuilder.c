...
void test_pack_packbuilder__extra_file_checks(void)
{
	...
	cl_git_pass(git_str_joinpath(&path, CLAR_FIXTURE_PATH, PACKFILE_WITH_KEEPFILE));
	cl_assert(git_fs_path_exists(path.ptr));
	git_packfile_alloc(&pack_file, path.ptr, GIT_OID_DEFAULT);
	...
	cl_git_pass(git_str_joinpath(&path, CLAR_FIXTURE_PATH, PACKFILE_WITH_PROMISORFILE));
	cl_assert(git_fs_path_exists(path.ptr));
	git_packfile_alloc(&pack_file, path.ptr, GIT_OID_DEFAULT);
